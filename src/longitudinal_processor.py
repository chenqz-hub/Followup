...
from longitudinal_models import LongitudinalPatientRecord, TimePointData, LongitudinalFollowupRecord
...
class LongitudinalEventProcessor:
    ...
    EVENT_TYPE_KEYWORDS = {
        ...
        'cardiovascular_event': ['心血管', 'cardiovascular', '事件', 'event', 'acute', 'myocardial', 'infarction', 'stroke', 'unstable'],
        'coronary_intervention': ['冠脉', 'coronary', 'intervention', 'angiography', 'stent', 'angioplasty', '介入'],
    ...
    def process_followup(self, patient_record: LongitudinalPatientRecord) -> LongitudinalFollowupRecord:
        """
        ...
        coronary_tracking = {
            ...
            'revascularization_treatment': {'date': None, 'time_point': None, 'has': False, 'type': None, 'detail': None},
        }
        ...
        for time_point in sorted_time_points:
            ...
            self._track_coronary_procedures(time_point, coronary_tracking, patient_record.enrollment_date)
        ...
        endpoint_occurred, survival_time, endpoint_desc = self._calculate_endpoint_outcome(
            event_tracking, 
            patient_record.enrollment_date, 
            patient_record.latest_followup_date
        )
        ...
        followup_record = LongitudinalFollowupRecord(
            ...
            # 各类事件首次发生信息
            first_death_date=event_tracking['death']['date'] or event_tracking['cardiac_death']['date'],
            first_death_time_point=event_tracking['death']['time_point'] or event_tracking['cardiac_death']['time_point'],
            days_to_first_death=event_tracking['death']['days'] or event_tracking['cardiac_death']['days'],
            first_mi_date=event_tracking['mi']['date'] or event_tracking['nonfatal_mi']['date'],
            first_mi_time_point=event_tracking['mi']['time_point'] or event_tracking['nonfatal_mi']['time_point'],
            ...
            first_revascularization_date=event_tracking['revascularization']['date'] or event_tracking['target_vessel_revascularization']['date'],
            first_revascularization_time_point=event_tracking['revascularization']['time_point'] or event_tracking['target_vessel_revascularization']['time_point'],
            days_to_first_revascularization=event_tracking['revascularization']['days'] or event_tracking['target_vessel_revascularization']['days'],
            first_hospitalization_date=event_tracking['hospitalization']['date'] or event_tracking['cardiac_hospitalization']['date'],
            first_hospitalization_time_point=event_tracking['hospitalization']['time_point'] or event_tracking['cardiac_hospitalization']['time_point'],
            days_to_first_hospitalization=event_tracking['hospitalization']['days'] or event_tracking['cardiac_hospitalization']['days'],
            # 冠脉相关检查/治疗
            ...
            first_coronary_angiography_time_point=coronary_tracking['coronary_angiography']['time_point'],
            ...
            first_coronary_intervention_time_point=coronary_tracking['coronary_intervention']['time_point'],
            ...
            first_revascularization_treatment_date=coronary_tracking['revascularization_treatment']['date'],
            first_revascularization_treatment_time_point=coronary_tracking['revascularization_treatment']['time_point'],
            first_revascularization_treatment_type=coronary_tracking['revascularization_treatment']['type'],
            first_revascularization_treatment_detail=coronary_tracking['revascularization_treatment']['detail'],
    ...
    def _calculate_endpoint_outcome(self, event_tracking: Dict, enrollment_date: date, 
                                   latest_date: date) -> Tuple[bool, int, str]:
        """
        根据选定的终点事件计算生存结局
        ...
        if self.endpoint == 'death':
            ...
            if event_tracking['death']['date'] or event_tracking['cardiac_death']['date']:
                ...
                endpoint_date = event_tracking['death']['date'] or event_tracking['cardiac_death']['date']
    ...
    def _identify_event(self, time_point: TimePointData) -> Tuple[Optional[str], Optional[date]]:
        ...
        if time_point.cardiovascular_event is not None:
            try:
                ...
                if self._contains_keywords(str(time_point.cardiovascular_event), 'cardiovascular_event'):
        ...
        if time_point.coronary_intervention is not None:
            ...
            if interv_code in [1, 1.0]:  # 假设1表示"有介入"
                return ('coronary_intervention', time_point.intervention_date or time_point.visit_date)
            if self._contains_keywords(str(time_point.coronary_intervention), 'coronary_intervention'):
                return ('coronary_intervention', time_point.intervention_date or time_point.visit_date)
    ...
    def _track_coronary_procedures(self, time_point: TimePointData, tracking: Dict, enrollment_date: Optional[date]) -> None:
        """
        追踪冠脉相关检查/治疗的首次发生
        ...
        if time_point.coronary_intervention is not None:
            ...
            if interv_code in [1, 1.0]:
                ...
                if not tracking['coronary_intervention']['has']:
                    ...
                    tracking['coronary_intervention']['date'] = time_point.intervention_date or visit_date
                ...
                if not tracking['coronary_angiography']['has']:
                    ...
                    tracking['coronary_angiography']['date'] = time_point.intervention_date or visit_date
        ...
        # 检查血运重建治疗字段
        if hasattr(time_point, 'revascularization_treatment') and time_point.revascularization_treatment is not None:
            ...
            if revasc_code in [1, 1.0]:
                if not tracking['revascularization_treatment']['has']:
                    ...
                    tracking['revascularization_treatment']['date'] = time_point.revascularization_date or visit_date
                    tracking['revascularization_treatment']['time_point'] = time_point.time_point
                    tracking['revascularization_treatment']['type'] = time_point.revascularization_type if hasattr(time_point, 'revascularization_type') else None
                    tracking['revascularization_treatment']['detail'] = time_point.revascularization_detail if hasattr(time_point, 'revascularization_detail') else None
    ...
    def process_batch(self, patient_records: List[LongitudinalPatientRecord]) -> List[LongitudinalFollowupRecord]:
        """
        批量处理多个患者的纵向随访数据
