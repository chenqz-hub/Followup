...
class TimePointData(BaseModel):
    ...
    event_types: List[str] = Field(default_factory=list, description="解析后的事件类型列表，如['angina', 'hospitalization']")
...
class LongitudinalPatientRecord(BaseModel):
    ...
    # 多时间点数据
    time_points: List[TimePointData] = Field(default_factory=list, description="所有时间点的随访数据")
...
class LongitudinalFollowupRecord(BaseModel):
    ...
    first_revascularization_time_point: Optional[str] = Field(None, description="首次血运重建时间点")
    days_to_first_revascularization: Optional[int] = Field(None, description="首次血运重建距入组天数")
    ...
    first_coronary_angiography_time_point: Optional[str] = Field(None, description="首次冠脉造影时间点")
    ...
    first_coronary_intervention_time_point: Optional[str] = Field(None, description="首次介入治疗时间点")
    ...
    first_revascularization_treatment_date: Optional[date] = Field(None, description="首次血运重建治疗日期")
    first_revascularization_treatment_time_point: Optional[str] = Field(None, description="首次血运重建治疗时间点")
    first_revascularization_treatment_type: Optional[str] = Field(None, description="首次血运重建治疗类型")
    first_revascularization_treatment_detail: Optional[str] = Field(None, description="首次血运重建治疗详情")
    ...
    endpoint_event: Optional[str] = Field(None, description="生存分析终点事件类型（death/mace/mi/any_event等）")
    ...
    def to_flattened_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'days_to_first_event': self.days_to_first_event,
            # 各类事件首次发生信息
            'first_death_date': self.first_death_date.isoformat() if self.first_death_date else None,
            ...
            'first_angina_date': self.first_angina_date.isoformat() if self.first_angina_date else None,
            'first_angina_time_point': self.first_angina_time_point,
            'days_to_first_angina': self.days_to_first_angina,
            'first_heart_failure_date': self.first_heart_failure_date.isoformat() if self.first_heart_failure_date else None,
            ...
            'first_revascularization_date': self.first_revascularization_date.isoformat() if self.first_revascularization_date else None,
            ...
            'first_hospitalization_date': self.first_hospitalization_date.isoformat() if self.first_hospitalization_date else None,
            ...
            'first_coronary_ct_date': self.first_coronary_ct_date.isoformat() if self.first_coronary_ct_date else None,
            'first_coronary_ct_time_point': self.first_coronary_ct_time_point,
            'has_coronary_angiography': self.has_coronary_angiography,
            'first_coronary_angiography_date': self.first_coronary_angiography_date.isoformat() if self.first_coronary_angiography_date else None,
            ...
            'first_coronary_intervention_date': self.first_coronary_intervention_date.isoformat() if self.first_coronary_intervention_date else None,
            ...
            'first_coronary_bypass_date': self.first_coronary_bypass_date.isoformat() if self.first_coronary_bypass_date else None,
            ...
            'first_revascularization_treatment_date': self.first_revascularization_treatment_date.isoformat() if self.first_revascularization_treatment_date else None,
